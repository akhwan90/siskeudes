    ...
    <Card :title="'Kegiatan'">
        <form @submit.prevent="submit" class="py-2">
            ...
            <div class="mb-2 flex">
                ...
                <input v-model="data.kode_bidang" ref="inputRef" type="text" class="border px-2 py-1" required :disabled="disable"/>
            </div>
            <div class="mb-2 flex">
                <label class="w-2/12">Kode Sub Bidang</label>
                <input v-model="data.kode_sub_bidang" type="text" class="border px-2 py-1" required :disabled="disable"/>
            </div>
            <div class="mb-2 flex">
                <label class="w-2/12">Kode Kegiatan</label>
                <input v-model="data.kode_kegiatan" type="text" class="border px-2 py-1" required :disabled="disable"/>
            </div>
            <div class="mb-2 flex">
                <label class="w-2/12">Nama Sub Bidang</label>
            ...
            <a :href="`/parameter/subbidang/${data.kode_sub_bidang}`" class="inline-block bg-slate-400 text-white px-4 py-2">Kembali</a>
        ...
        <table class="border w-full">
            <thead>
                <tr>
                    ...
                    <th class="border px-2 py-2" width="70%">Kegiatan</th>
            ...
            <tbody>
                <tr v-for="item in datas" :key="item.kode_gabung">
                    ...
                    <td class="border px-2 py-1">
                        <a class="inline-block bg-blue-400 px-2 hover:bg-blue-600 py-1 mr-2" :href="`/parameter/kegiatan/${item.kode_bidang}/${item.kode_sub_bidang}`">Kegiatan</a>
...
export default {
    ...
    mounted() {
        this.data.kode_bidang = this.$route.params.kdBidang;
        this.data.kode_sub_bidang = this.$route.params.kdSubbidang;
        this.loadData();
    },
    methods: {
        tambah() {
        ...
        edit(item) {
            ...
            this.data.kode_sub_bidang = item.kode_sub_bidang;
            this.data.kode_kegiatan = item.kode_kegiatan;
        ...
        async remove(item) {
            if (confirm("Anda yakin")) {
                try {
                    await axios.delete(`/admin/parameter/kegiatan/${item.kode_bidang}/${item.kode_sub_bidang}/remove/${item.kode_kegiatan}`, {
        ...
        async loadData() {
            try {
                const response = await axios.get(`/admin/parameter/kegiatan/${this.data.kode_bidang}/${this.data.kode_sub_bidang}`, {
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                });
        ...
        async submit() {
            ...
            try {
                const response = await axios.post(`/admin/parameter/kegiatan/${this.data.kode_bidang}/${this.data.kode_sub_bidang}/save`, formData, {
