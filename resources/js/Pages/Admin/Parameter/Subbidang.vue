<template>
    <Card :title="'Sub Bidang'">
        <form @submit.prevent="submit" class="py-2">
            ...
            <a :href="`/parameter/bidang`" class="inline-block bg-slate-400 text-white px-4 py-2">Kembali</a>
...
export default {
    ...
    methods: {
        ...
        batal() {
            this.disable =!this.disable;
            this.data.kode_sub_bidang = null;
            this.data.nama = null;
            // this.loadData();
        },
        async remove(item) {
            if (confirm("Anda yakin")) {
                try {
                    await axios.delete(`/admin/parameter/subbidang/${item.kode_bidang}/remove/${item.kode_sub_bidang}`, {
        ...
        async loadData() {
            try {
                const response = await axios.get(`/admin/parameter/subbidang/${this.data.kode_bidang}`, {
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                });
        ...
        async submit() {
            ...
            try {
                const response = await axios.post(`/admin/parameter/subbidang/${this.data.kode_bidang}/save`, formData, {
